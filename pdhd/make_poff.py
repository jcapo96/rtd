...
allBool = True
system = "tgrad"
start_date = datetime(2024, 5, 28, 22, 0, 0) #write here the start datetime
end_date = datetime(2024, 5, 31, 12, 30, 0) #write here the end datetime
configuration = "baseline"

path = f"/eos/user/j/jcapotor/PDHDdata/poff/calib/POFF_{start_date.strftime('%Y-%m-%d')}T{start_date.strftime('%H:%M:%S')}_{end_date.strftime('%Y-%m-%d')}T{end_date.strftime('%H:%M:%S')}"
ref = "40525"
...
m = MakeData(detector="np04", all=allBool, system=system,
                    ...
                    clockTick=60,
                    pathToSaveData=F"/eos/user/j/jcapotor/PDHDdata/poff/data/",
                    ref=ref, configuration=configuration, FROM_CERN=True)
m.make()
# m.getData()
